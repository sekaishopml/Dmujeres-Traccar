...
object Envelope {
    ...
    fun buildPosition(
        ...
        bootId: String? = null,
        // FASE 5: identidad de la jornada (epoch ms del journeyStartAt) para el
        // motor de continuidad servidor. 0/null = sin jornada.
        journeyId: Long? = null,
        ...
        // Estado del acelerómetro (MotionSensor.State): STATIONARY|MOVING|UNKNOWN.
        // AUXILIAR: el server NO lo usa como autoridad de posición ni de parada;
        // el estado calculado server-side es MotionStateV2 (otro concepto).
        motionState: String? = null,
        // R5: evidencia del primer fix tras un CAPTURE_GAP (opcionales, additive;
        // el server los guarda como atributos y el replay los puede mostrar).
        gapMs: Long? = null,
        gapMovementDuring: Boolean? = null,
        gapRescueAttempted: Boolean? = null,
        gapRescueRecovered: Boolean? = null,
    ): String {
        ...
        if (journeyId != null && journeyId > 0L) payload.put("journeyId", journeyId)
        ...
        if (!motionState.isNullOrBlank() && motionState in setOf("STATIONARY", "MOVING", "UNKNOWN")) {
            payload.put("motionState", motionState)
        }
        // R5: evidencia del primer fix tras un CAPTURE_GAP (additive, schema:1
        // tolera extras; el replay los muestra como evidencia, no como ruta).
        if (gapMs != null && gapMs > 0L) payload.put("gapMs", gapMs)
        if (gapMovementDuring != null) payload.put("movementDuringGap", gapMovementDuring)
        if (gapRescueAttempted != null) payload.put("rescueAttempted", gapRescueAttempted)
        if (gapRescueRecovered != null) payload.put("rescueRecovered", gapRescueRecovered)
    ...
    fun buildPresence(
        ...
        // R9: el colaborador tocó el botón "Actualizar" (auditable en el panel).
        otaManualPressedAt: Long = 0L,
        // R9: fallos del proveedor fused (Google) en esta jornada. Indica ROMs
        // donde conviene el GPS del sistema (AOSP) como primario.
        fusedFailures: Int = 0,
    ): String {
        val payload = JSONObject()
        payload.put("pending", pending)
        ...
        payload.put("gps", gps)
        if (otaManualPressedAt > 0) {
            payload.put("otaManualPressedAt", otaManualPressedAt)
        }
        if (fusedFailures > 0) {
            payload.put("fusedFailures", fusedFailures)
        }
        ...
        if (journeyStatus == MobileProtocol.JOURNEY_STATUS_STARTED) payload.put("journeyStarted", true)
        if (journeyStatus == MobileProtocol.JOURNEY_STATUS_ENDED) payload.put("journeyEnded", true)
