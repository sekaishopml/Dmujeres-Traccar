...
object PositionOutboxDispatcher {
    ...
    object HttpTransport : Transport {
        override suspend fun post(ctx: DispatchContext, items: List<PendingPosition>): PostResult {
            ...
            val connection = try {
                ...
                (URL("${ctx.webBaseUrl}${MobileProtocol.PATH_POSITIONS}").openConnection() as HttpURLConnection).apply {
                    requestMethod = "POST"
    ...
    private suspend fun flushLocked(
        ...
        val pending = try {
            ...
            // R8: un fallo de DB no puede parecer "nada que enviar".
            com.dmujeres.traccar.platform.SentryLog.breadcrumb("outbox", "db_error_allDue", e.message ?: e.javaClass.simpleName)
