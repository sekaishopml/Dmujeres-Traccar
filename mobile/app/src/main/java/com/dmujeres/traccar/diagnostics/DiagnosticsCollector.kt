...
data class DiagnosticsSources(
    ...
    // ---- F0: device health (telemetry de flota de compatibilidad) ----------
...
object DiagnosticsCollector {
    ...
    fun compact(s: DiagnosticsSources): Map<String, Any?> = mapOf(
        ...
        "deviceHealth" to buildMap {
            nonBlank(s.manufacturer)?.let { put("manufacturer", it) }
            nonBlank(s.model)?.let { put("model", it) }
            nonBlank(s.androidVersion)?.let { put("androidVersion", it) }
            put("screenOn", s.screenOn)
            nonBlank(s.motionState)?.let { put("motion", it) }
            nonBlank(s.oemKey.orEmpty())?.let { put("oem", it) }
            put("oemConfirmed", s.oemConfirmed)
            nonBlank(s.readinessVerdict)?.let { put("readiness", it) }
            nonBlank(s.continuityState)?.let { put("continuity", it) }
            nonBlank(s.continuityCause)?.let { put("continuityCause", it) }
            nonBlank(s.recoveryState)?.let { put("recovery", it) }
        },
        "health" to buildMap {
    ...
    fun sourcesFrom(
        ...
        return DiagnosticsSources(
            ...
            manufacturer = runCatching { DeviceCaps.from(context).manufacturer }.getOrDefault(""),
            model = runCatching { DeviceCaps.from(context).model }.getOrDefault(""),
            androidVersion = runCatching { DeviceCaps.from(context).androidRelease }.getOrDefault(""),
            screenOn = runCatching {
                val pm = context.getSystemService(android.content.Context.POWER_SERVICE)
                    as? android.os.PowerManager
                pm?.isInteractive ?: false
            }.getOrDefault(false),
            motionState = runCatching { MotionSensor.currentState().name }.getOrDefault("UNKNOWN"),
            oemKey = runCatching { VendorSettings.currentVendor() }.getOrNull(),
            oemConfirmed = runCatching { config.vendorGuideDone }.getOrDefault(false),
            readinessVerdict = runCatching {
                com.dmujeres.traccar.readiness.DeviceReadinessChecker.evaluate(context).verdict
            }.getOrDefault(""),
            continuityState = runCatching { config.continuityState }.getOrDefault(""),
            continuityCause = runCatching { config.continuityCause }.getOrDefault(""),
            recoveryState = runCatching { config.lastRecoveryResult }.getOrDefault(""),
        )
    }
