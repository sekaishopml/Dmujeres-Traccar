...
class EnvelopeTest {
    ...
    @Test
    fun positionCarriesMotionStateWhenKnown() {
        val payload = JSONObject(
            Envelope.buildPosition(
                messageId = "m", deviceId = "d", sequence = 12L,
                latitude = 19.4326, longitude = -99.1332, accuracy = 5.0,
                speed = 0.0, bearing = 0.0, altitude = 0.0,
                observedAt = "2026-01-01T00:00:00Z", pending = 0, battery = 80,
                network = "wifi", motionState = "STATIONARY",
            )
        ).getJSONObject("payload")
        assertEquals("STATIONARY", payload.getString("motionState"))
    }

    @Test
    fun positionOmitsMotionStateWhenUnknownValue() {
        val payload = JSONObject(
            Envelope.buildPosition(
                messageId = "m", deviceId = "d", sequence = 13L,
                latitude = 19.4326, longitude = -99.1332, accuracy = 5.0,
                speed = 0.0, bearing = 0.0, altitude = 0.0,
                observedAt = "2026-01-01T00:00:00Z", pending = 0, battery = 80,
                network = "wifi", motionState = "WARP",
            )
        ).getJSONObject("payload")
        assertFalse(payload.has("motionState"))
    }

    @Test
    fun positionOmitsMotionStateWhenAbsent() {
        val payload = JSONObject(
            Envelope.buildPosition(
                messageId = "m", deviceId = "d", sequence = 14L,
                latitude = 19.4326, longitude = -99.1332, accuracy = 5.0,
                speed = 0.0, bearing = 0.0, altitude = 0.0,
                observedAt = "2026-01-01T00:00:00Z", pending = 0, battery = 80,
                network = "wifi",
